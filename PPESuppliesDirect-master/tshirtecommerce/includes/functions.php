...
class dg{
	...
	public function prices($data, $add_tax = true)
	{
		...
		else
		{
			...
			$price_extra = $this->getPrintingType( $data['print_type'] )->price_extra;
			$pure_per_item = $per_item * $vat * $quantity;
			...
			if( $quantity > 9 ) {
				...
				$total->old 	= number_format($per_item * $quantity, $number, $price_decimal, $price_thousand);
			}
			$total->sale 	= number_format($per_item * $quantity, $number, $price_decimal, $price_thousand);
			$total->item 	= number_format($total->sale / $quantity, $number, $price_decimal, $price_thousand);
			if( $data['digitisation'] === 'on' ) {
				if( $total->printing > 0 ) {
					$total->antondrob_cust_price = number_format($total->printing * $quantity - $price_extra, $number, $price_decimal, $price_thousand);
					$total->price_extra = $price_extra;
				}else {
					$total->antondrob_cust_price = 0;
					$total->price_extra = 0;
				}
			}else {
				if( $total->printing > 0 ) {
					$total->antondrob_cust_price = number_format( ( $total->printing + $total->clipart ) * $quantity, $number, $price_decimal, $price_thousand );
					$total->price_extra = 0;
				}else {
					$total->antondrob_cust_price = 0;
					$total->price_extra = 0;
			...
			$total->antondrob_product_price = number_format( $total->sale - $total->antondrob_cust_price - $total->price_extra, $number, $price_decimal, $price_thousand );
