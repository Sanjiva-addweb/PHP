	...
	ajax:{
		...
		getPrice: function(){
			...
			jQuery.ajax({
				...
				processData: false,
				dataType: "json",
				url: siteURL + "ajax.php?type=prices&parent_product_id=" + findGetParameter('parent') + "&vat=" + vatData() + "&dig=" + dig(),
				...
				if (data != '')
				{
					...
					if (typeof data.antondrob_product_price != 'undefined') {
						jQuery('.price-product-number').text(data.antondrob_product_price);
					}else {
						jQuery('.price-product-number').text(0);
					}
					if (typeof data.antondrob_cust_price != 'undefined') {
						jQuery('.price-cust-number').text(data.antondrob_cust_price);
					}else {
						jQuery('.price-cust-number').text(0);
					}
					if(typeof data.price_extra != 'undefined') {jQuery('.digitisation').text(data.price_extra);}else {jQuery('.digitisation').text(0);}
				}
			}).always(function(){
				lable.html(title);				
				design.print.colors();
		...
		addToCart: function(){
			...
			jQuery.ajax({
				...
				dataType: "json",
				contentType: "application/json; charset=utf-8",	
				url: siteURL + "ajax.php?type=addCart&parent_product_id=" + findGetParameter('parent')
			}).done(function( data ){
...
function dig() {
	if ( jQuery('input[name="digitisation"]').is(':checked') ) {
		return 'on';
	}else {
		return 'off';
	}
