...
if ( !function_exists('yozi_woocommerce_enqueue_styles') ) {
    function yozi_woocommerce_enqueue_styles() {
        ...
        $options = array(
            ...
            'template' => apply_filters( 'yozi_autocompleate_search_template', '<a href="{{url}}" class="media autocompleate-media"><div class="media-left media-middle"><img src="{{image}}" class="media-object" height="40" width="40"></div><div class="media-body media-middle" style="width:auto;"><h4>{{{title}}}</h4><p class="price">{{{price}}}</p></div></a>' ),
...
function yozi_woocommerce_get_ajax_products() {
    ...
    if ( $loop->have_posts() ) {
        ...
        echo'<div class="no-data-product-tab">No product available.</div>';
    }
    exit();
}
