				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<div class="media widget-product<?php echo ($bulk_products[$product_id]['hidden']) ? ' bulk-item-hidden' : ''; ?>">
							...
							<div class="cart-main-content media-body media-middle<?php echo ( !$bulk_products[$product_id]['hidden'] && has_term( 'bulk-deals', 'product_cat', $product_id ) ) ? ' bulk-remove' : ''; ?>" data-product-id="<?php echo trim($product_id); ?>">
								...
								if( has_term( 'bulk-deals', 'product_cat', $product_id ) ){
									...
									echo '<span class="quantity text-theme">' . $cart_item['quantity'] . ' x ' . wc_price( wc_get_price_excluding_tax( $_product, array( 'qty' => $cart_item['quantity'] ) ) ) . '</span>';
								}								
								?>
								<h3 class="name">
								...
								<?php
								    // echo apply_filters(
								        ...
								        // $cart_item_key
								    // );
									$link = wc_get_page_permalink( 'cart' ).'?custom-remove-item='.$cart_item_key.'&product-id='.$variation_id.'&quantity='.$cart_item['quantity'].'" class="remove" title="'.esc_html__( 'Remove this item', 'yozi' );
									echo '<a href="'.$link.'"> <i class="ion-android-close"></i> </a>';
								?>
