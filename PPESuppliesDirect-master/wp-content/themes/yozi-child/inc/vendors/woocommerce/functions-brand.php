...
if ( !class_exists("Yozi_Woo_Brand") ) {
	class Yozi_Woo_Brand {

		public static function init() {
			// brand
			$tax = yozi_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'yozi' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'yozi' ); ?></label></th>
				<td>
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'yozi' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img style="max-width: 60px;" src="' . esc_url( $image ) . '" alt="'.esc_html__( 'Image', 'yozi' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = yozi_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Yozi_Woo_Brand', 'init') );
}