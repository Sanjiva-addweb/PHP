    ...
    class WC_Gateway_Sagepay_Direct extends _WC_Gateway_Sagepay_Direct {
        ...
        public function __construct() {
			...
			$this->sage_cardtypes = apply_filters( 'wc_sagepaydirect_cardtypes', array(
				...
				'Diners Club'		=> __( 'Diners Club', 'woocommerce_sagepayform' ),
            ...
            if ( isset($_POST['MD']) && ( isset($_POST['PARes']) || isset($_POST['PaRes']) ) ) {
				...
				try {
					...
					if( isset( $sageresult['Status']) && $sageresult['Status']!= '' && !in_array( $sageresult['Status'], $this->failed_3d_secure_status ) ) {
						...
						if( isset( $sageresult['redirect'] ) ) {
							$redirect_url = $sageresult['redirect'];
						}
...
?>
		...
		function sagepaydirect_ssl_check() {
			...
			if( $this->enabled == "yes" ) {
		    	...
		    	if ( !$this->is_secure() ) {
		     		echo '<div class="error notice sagepaydirect-ssl-nag is-dismissible"><p>'. __( 'SagePay Direct is enabled and your checkout is not secure! Please enable SSL and ensure your server has a valid SSL certificate before going live.', 'woocommerce_sagepayform' ) .'</p></div>';
		    	}
        ...
        function process_pre_order_release_payment( $order ) {
            ...
            if ( 'OK' != $result['Status'] ) {
                ...
                // complete the order
                $order->set_status( ($order->needs_processing() ? 'processing' : 'completed'), __('Payment completed', 'woocommerce-spusa') . ( isset( $result['message'] ) ? '<br />Approval Msg: ' . $result['message'] : NULL ) . '<br />' );
		...
		/**
		 * Test is the checkout is using HTTPS
		 */
