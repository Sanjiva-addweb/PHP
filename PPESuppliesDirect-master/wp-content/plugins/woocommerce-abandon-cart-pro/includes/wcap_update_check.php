    ...
    class Wcap_Update_Check {
        ...
        public static function wcap_update_db_check() {
            ...
            if ( $woocommerce_ac_plugin_version != Wcap_Common::wcap_get_version() ) {
                ...
                update_option( 'woocommerce_ac_db_version', '8.1.0' );
                ...
                $get_auto = $wpdb->get_results( "SHOW TABLE STATUS FROM `" . DB_NAME . "` WHERE `name` LIKE '" . WCAP_TINY_URLS . "'" );
                if( isset( $get_auto[0]->Auto_increment ) && $get_auto[0]->Auto_increment < 1000000 ) {
                    $wpdb->query( "ALTER TABLE  ". WCAP_TINY_URLS . " AUTO_INCREMENT = 1000000" );
                }

                // if the tables are not indexed, add them
                $get_index_cart = "SHOW INDEX FROM " . WCAP_ABANDONED_CART_HISTORY_TABLE . " WHERE Key_name = 'id'";
                $check_index_cart = $wpdb->get_results( $get_index_cart );
                if( is_array( $check_index_cart ) && count( $check_index_cart ) == 0 ) {
                    $add_index_cart = "CREATE UNIQUE INDEX id ON " . WCAP_ABANDONED_CART_HISTORY_TABLE . " (id)";
                    $wpdb->query( $add_index_cart );
                }

                $get_index_guest = "SHOW INDEX FROM " . WCAP_GUEST_CART_HISTORY_TABLE . " WHERE Key_name = 'id'";
                $check_index_guest = $wpdb->get_results( $get_index_guest );
                if( is_array( $check_index_guest ) && count( $check_index_guest ) == 0 ) {
                    $add_index_guest = "CREATE UNIQUE INDEX id on " . WCAP_GUEST_CART_HISTORY_TABLE . " (id)";
                    $wpdb->query( $add_index_guest );
                }

                $get_index_sent = "SHOW INDEX FROM " . WCAP_EMAIL_SENT_HISTORY_TABLE . " WHERE Key_name = 'order_id'";
                $check_index_sent = $wpdb->get_results( $get_index_sent );
                if( is_array( $check_index_sent ) && count( $check_index_sent ) == 0 ) {
                    $add_index_sent = "CREATE INDEX order_id on " . WCAP_EMAIL_SENT_HISTORY_TABLE . " (abandoned_order_id)";
                    $wpdb->query( $add_index_sent );
                }
                
                if ( 'yes' !== get_option( 'ac_pro_user_cleanup' ) ) {
                    $query_cleanup = "UPDATE `" . WCAP_GUEST_CART_HISTORY_TABLE . "` SET 
                ...
                // @since 7.14.0
                if ( ! $wpdb->get_var( "SHOW COLUMNS FROM `".WCAP_EMAIL_TEMPLATE_TABLE."` LIKE 'cart_rules';" ) ) {
                    $wpdb->query( "ALTER TABLE ".WCAP_EMAIL_TEMPLATE_TABLE." ADD `cart_rules` varchar(50) COLLATE utf8_unicode_ci NOT NULL AFTER `wc_template_filter`;" );
                }

                if ( ! $wpdb->get_var( "SHOW COLUMNS FROM `".WCAP_EMAIL_TEMPLATE_TABLE."` LIKE 'product_ids';" ) ) {
                    $wpdb->query( "ALTER TABLE ".WCAP_EMAIL_TEMPLATE_TABLE." ADD `product_ids` varchar(100) COLLATE utf8_unicode_ci NOT NULL AFTER `cart_rules`;" );
                }

                // Move all the email template data that is saved in post meta table to the email templates table. 
                if( 'yes' != get_option( 'wcap_move_templates_data' ) ) {
                    $columns_added = false; 
                    if ( ! $wpdb->get_var( "SHOW COLUMNS FROM `".WCAP_EMAIL_TEMPLATE_TABLE."` LIKE 'send_emails_to'" ) ) {
                        $wpdb->query( "ALTER TABLE " . WCAP_EMAIL_TEMPLATE_TABLE . " ADD `send_emails_to` VARCHAR(300) NOT NULL AFTER `product_ids`" );
                        $columns_added = true;
                    }

                    if ( ! $wpdb->get_var( "SHOW COLUMNS FROM `".WCAP_EMAIL_TEMPLATE_TABLE."` LIKE 'activated_time'" ) ) {
                        $wpdb->query( "ALTER TABLE " . WCAP_EMAIL_TEMPLATE_TABLE . " ADD `activated_time` int(11) NOT NULL AFTER `send_emails_to`" );
                        $columns_added = true;
                    }

                    //
                    $email_templates = $wpdb->get_col( "SELECT id FROM " . WCAP_EMAIL_TEMPLATE_TABLE );
                    if( is_array( $email_templates ) && $columns_added ) {
                        ...
                        foreach( $email_templates as $id ) {
                            ...
                            $wpdb->update( WCAP_EMAIL_TEMPLATE_TABLE, $update_array, array( 'id' => $id ) );

                        }

                        // delete all the data from post meta
                        $wpdb->query( "DELETE FROM `" . $wpdb->prefix . "postmeta` WHERE meta_key = 'wcap_template_time'" );
                        $wpdb->query( "DELETE FROM `" . $wpdb->prefix . "postmeta` WHERE meta_key = 'wcap_email_action'" );
                        $wpdb->query( "DELETE FROM `" . $wpdb->prefix . "postmeta` WHERE meta_key = 'wcap_other_emails'" ); 
