    ...
    class Wcap_Ajax{
        ...
        public static function wcap_import_lite_data () {
            ...
            if ( 'true' === $_POST['wcap_import_template'] ) {
                ...
                $wcap_templates_data_query = "INSERT INTO `". WCAP_EMAIL_TEMPLATE_TABLE ."` ( `subject`, `body`, `is_active`, `frequency`, `day_or_hour`, `template_name`, `is_wc_template`, `default_template`, `wc_email_header`, `send_emails_to`, `activated_time` ) 
                    SELECT `subject`, `body`, '0', `frequency`, `day_or_hour`, `template_name`, `is_wc_template`, `default_template`, `wc_email_header`, '" . $send_emails . "', '' FROM `".$wpdb->prefix."ac_email_templates_lite` "; 
        ...
        public static function wcap_save_guest_data() {
            ...
            if ( ! is_user_logged_in() && ( false == $get_restricted_ip_address && false == $get_restricted_email_address && false == $get_restricted_domain_address ) ) {
                ...
                if ( $wcap_user_id !='' && 
                    ...
                    if ( $billing_email != '' && 
                         isset( $billing_email_post ) && 
                         $billing_email_post != '') {

                        Wcap_Ajax::wcap_update_guest_record_on_same_email( $wcap_wc_session_key, $wc_shipping_charges, $guest_details );
                        wp_die();
                    } else {
                        /**
                        ...
                        $results_existing_guest_phone = $wpdb->get_results( $wpdb->prepare( $query_existing_guest_phone, $billing_phone_post, $abandoned_order_id ) );

                        if ( isset( $results_existing_guest_phone ) && 
                             count ( $results_existing_guest_phone ) > 0 && 
                             isset( $billing_phone_post ) && 
                             $billing_phone_post != '') {

                            Wcap_Ajax::wcap_update_guest_record_on_same_phone( $wcap_wc_session_key, $wc_shipping_charges, $guest_details );
                            wp_die();
                        }
                    }
                }else {
                    ...
                    if( isset( $billing_email_post ) && 
                        ...
                        $email_check = sanitize_text_field( $_POST[ 'billing_email' ] );

                        $session_email = wcap_get_cart_session( 'wcap_guest_email' );
                        
                        if( $session_email != $billing_email_post ) {
                            $email_check = $session_email;
                            wcap_set_cart_session( 'wcap_guest_email', $billing_email_post );
                        }
                    ...
                    }elseif ( isset( $billing_phone_post ) && 
                        $billing_phone_post != '' &&
                        isset( $_POST['wcap_record_added'] ) &&
                        $_POST['wcap_record_added'] !== 'false' &&
                    ...
                    }elseif ( isset( $billing_first_name_post ) && 
                        $billing_first_name_post != '' &&
                        isset( $_POST['wcap_record_added'] ) &&
                        $_POST['wcap_record_added'] !== 'false' &&
                        ...
                        if( isset( $results_existing_guest_first_name ) && 
                            ...
                            isset( $billing_first_name_post ) && 
                            $billing_first_name_post != '' ) {

                            Wcap_Ajax::wcap_update_guest_record_on_same_first_name( $wcap_wc_session_key, $wc_shipping_charges, $guest_details );
                            wp_die();
                        }
                    }elseif ( isset( $billing_last_name_post ) && 
                        $billing_last_name_post != '' &&
                        isset( $_POST['wcap_record_added'] ) &&
                        $_POST['wcap_record_added'] !== 'false' &&
                        ...
                        if( isset( $results_existing_guest_last_name ) && 
                            ...
                            isset( $billing_last_name_post ) && 
                            $billing_last_name_post != '' ) {

                            Wcap_Ajax::wcap_update_guest_record_on_same_last_name( $wcap_wc_session_key, $wc_shipping_charges, $guest_details );
                            wp_die();
                        }
                    }elseif ( isset( $billing_email_post ) && $billing_email_post != '' || 
