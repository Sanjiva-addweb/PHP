...
class Wcap_TS_tracking {
	 ...
	 *
	 */
	public function __construct( $ts_plugin_prefix = 'wcap', $ts_plugin_name = 'Abandoned Cart Pro for WooCommerce', $ts_blog_post_link = 'https://www.tychesoftwares.com/docs/docs/abandoned-cart-pro-for-woocommerce/usage-tracking/', $ts_plugin_context = 'woocommerce-ac', $ts_plugin_url = WCAP_PLUGIN_URL, $setting_page = 'admin.php?page=woocommerce_ac_page&action=emailsettings', $add_setting = 'woocommerce_ac_page', $setting_section = 'ac_general_settings_section', $setting_register = 'woocommerce_ac_settings' ) {
	...
	public static function ts_add_cron_schedule( $schedules ) {
		$schedules[ 'once_in_week_wcap' ] = array(
	...
	public static function ts_schedule_cron_job () {
		if ( ! wp_next_scheduled( 'wcap_ts_tracker_send_event' ) ) {
			wp_schedule_event( time() + 604800, 'once_in_week_wcap', 'wcap_ts_tracker_send_event' );
		}
	}
...
return new Wcap_TS_tracking( 'wcap', 'Abandoned Cart Pro for WooCommerce', 'https://www.tychesoftwares.com/docs/docs/abandoned-cart-pro-for-woocommerce/usage-tracking/', 'woocommerce-ac', WCAP_PLUGIN_URL, 'admin.php?page=woocommerce_ac_page&action=emailsettings', 'woocommerce_ac_page', 'ac_general_settings_section', 'woocommerce_ac_settings' );