...
class Wcap_TS_Tracker {
	...
	*/
	 ...
	 */
	public function __construct( $ts_plugin_prefix = 'wcap', $ts_plugin_name = 'Abandoned Cart Pro for WooCommerce' ) {

		self::$plugin_prefix = $ts_plugin_prefix;
		self::$plugin_name   = $ts_plugin_name;

		add_action( 'wcap_ts_tracker_send_event', array( 'Wcap_TS_Tracker', 'ts_send_tracking_data' ) );
	}
	 ...
	 */
	public static function ts_send_tracking_data() {
	...
	private static function ts_get_tracking_data() {
		...
		$data = Wcap_Common::ts_add_plugin_tracking_data( $data );
...
return new Wcap_TS_Tracker( 'wcap', 'Abandoned Cart Pro for WooCommerce' );