...
<div class="form-group">
	<div class='col-sm-12 row allow_change_printing_type_parent'>
	...
	<div class='col-sm-12 row printting_type_category' <?php if(!isset($data->allow_change_printing_type)) echo "style='display:none;'"; ?>>
		<?php 
			if(count($print_types))
			{
				foreach($print_types as $key=>$val)
				{
					if($data->print_type == $key)
						echo '<div class="form-group allow_'.$key.'_printing" style="display: none;">';
					else
						echo '<div class="form-group allow_'.$key.'_printing">';
					
					echo '<label class="col-sm-5 control-label">'.$val.'</label>';
					
					echo '<div class="col-sm-6">';
					$attr = 'allow_'.$key.'_printing';
						
					if(isset($data->$attr) || $data->print_type == $key)
						echo '<input type="checkbox" name="product[allow_'.$key.'_printing]" value="1" checked="">';
					else
